                     ...
                     higher_order = TRUE)
{
  ...
  # Pre-sort univariate GeDS base-learner covariates once (reused across all
  # local-scoring / backfitting iterations); avoids per-fit formula parsing.
        ...
        suppressWarnings({
          fit <- tryCatch(
            if (!is.null(cache)) {
              ...
              uf <- UnivariateFitter(X = cache$xs, Y = partial_resid[cache$ord], Z = NULL,
                                     offset = rep(0, length(cache$ord)), weights = wz[cache$ord],
