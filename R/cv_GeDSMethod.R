...
avg_intknots <- function(object){
  ...
  n_iterations <- object$iters
  if (n_iterations == 0){
    avg_intknots_univbl<- avg_intknots_bivblX <- avg_intknots_bivblY <- 0
  } else {
    # Compute the average # of int. knots per boosting iteration
    total <- totalX <- totalY <- 0
    for (i in 1:n_iterations) {
      model_name <- paste0("model", i)
      best_bl <- object$models[[model_name]]$best_bl
      n_best_bl_vars <- length(object$args$base_learners[[best_bl$name]]$variables)
      # UNIVARIATE
      if (n_best_bl_vars == 1) {
        n_intknots <- best_bl$n_intknots
        # Add the current model's n_intknots to the total
        total <- total + n_intknots
        # BIVARIATE
      } else if (n_best_bl_vars == 2) {
        n_intknotsX <- best_bl$n_intknots$X
        n_intknotsY <- best_bl$n_intknots$Y
        # Add the current model's n_intknots to the total
        totalX <- totalX + n_intknotsX
        totalY <- totalY + n_intknotsY
      }
      
    }
    avg_intknots_univbl <- total / n_iterations
    avg_intknots_bivblX <- totalX / n_iterations
    avg_intknots_bivblY <- totalY / n_iterations
  }
...
# 
#   # Return the optimal internal_knots, max_iterations and shrinkage values
#   return(list("best_params" = best_params, "results_df" = results_df))
# }
...
#   X <- as.matrix(data[predictors])
# 
#   # Check if model_fun is one of the admissible functions
#   if (!(deparse(substitute(model_fun)) %in% c("NGeDSboost", "NGeDSgam"))) {
#     stop("model_fun must be one of the following: ", paste(admissible_functions, collapse=", "), ".")
#   }
# 
#   # Check if order is correctly set
#   n <- as.integer(n)
