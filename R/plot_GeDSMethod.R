                      ...
                      n = 3L, type = c("none", "polygon", "nci", "aci"), ...)
{
  ...
  if (x$type == "LM - Univ" || x$type == "GLM - Univ") {
    ...
    if (!requireNamespace("plot3D", quietly = TRUE)) {
      stop(
        "Package 'plot3D' is required for bivariate plots. Please install it first.",
        call. = FALSE
      )
    }
    ...
    if (!is.null(f)) {
      ...
      # Plot
      plot3D::persp3D(x = seq_valX, y = seq_valY, z = f_XY_val, phi = 25, theta = 50,
      ...
      plot3D::points3D(x = X, y = Y, z = Z, col = "black", pch = 19, add = TRUE)
      plot3D::points3D(x = X, y = Y, z = obj$predicted, col = "red", pch = 19, add = TRUE)
      legend("topright",
      ...
      plot3D::persp3D(x = newX, y = newY, z = f_XY_hat_val, phi = 25, theta = 50,
      ...
      plot3D::points3D(x = X[!tmp], y = Y[!tmp], z = Z[!tmp], col = "red", pch = 19, add = TRUE)
      plot3D::points3D(x = X[tmp], y = Y[tmp], z = Z[tmp], col = "blue", pch = 19, add = TRUE)
    ...
    if (NCOL(X_mat) == 1) {
      ...
      if  (!is.factor(X_mat)) {
        ...
        if (!is.null(f)) {
          ...
          line_arg_names <- c("col", "lty", "lwd", "type")
          line_args <- dots[names(dots) %in% line_arg_names]
          plot_args <- dots[!names(dots) %in% line_arg_names]

          f_data <- data.frame(
            x = X_mat,
            y = f(X_mat)
          )
          f_data <- f_data[order(f_data$x), ]
          ...
          plot_defaults <- list(
            ...
            ylab = ylab,
            xlim = range(X_mat),
            ylim = range(c(f_data$y, plot_data[[2]]), na.rm = TRUE),
            lwd = 2,
            col = "black"
          )

          plot_args <- modifyList(plot_defaults, plot_args)
          ...
          line_args <- modifyList(line_defaults, line_args)
      ...
      # Plot the perspective 3D surface defined by newX, newY, and the fitted values
      plot3D::persp3D(x = newX, y = newY, z = f_XY_hat_val, main = title, phi = 25, theta = 50,
      ...
      plot3D::segments3D(x0 = kntX, y0 = rep(min(newY), length(kntX)), z0 = rep(min(f_XY_hat_val), length(kntX)),
      ...
      plot3D::segments3D(x0 = rep(max(newX), length(kntY)), y0 = kntY, z0 = rep(min(f_XY_hat_val), length(kntY)),
