...
Gradient Boosting algorithms rank among the top models for tabular data, often being the go-to choice for ML competitions.
...
------------------------------------------------------------------------
Given a data sample $\{Y_i,X_i\}_{i=1}^N$, a differentiable loss function $L\left(Y,F(X)\right)$ and a stopping number of iterations $m_{\text{stop}}$, the generic FGB algorithm proceeds as follows:
...
\end{equation}
where $m^*$ ($\sim m_{\text{stop}}$) stands for the boosting iteration where the optimal fit $\hat{F}^{*}$ is achieved; $\boldsymbol{\hat{f}_{i}}$ denotes the optimal learner selected according to some pre-established criterion at the $i$th boosting iteration. Each function estimate $\hat{F}^j$ is then the cumulative sum of the estimates $f_{i}^{j}$ for each iteration where the corresponding base-learner, $j$, was selected to update the model $\widehat{F}$, i.e., for each iteration where $\boldsymbol{\hat{f}_{i}}=\hat{f}_{i}^{j}$.
...
- The stopping boosting iteration, $m_{\text{stop}}$. For example, [@Buhlmann2011](https://link.springer.com/book/10.1007/978-3-642-20192-9) suggest this can be determined via cross-validation.
...
There is a clear trade-off in the selection of $\nu$ and $m_{\text{stop}}$: smaller values of $\nu$ give rise to larger optimal $m_{\text{stop}}$-values, and viceversa ([@Friedman2001](https://doi.org/10.1214/aos/1013203451)).
